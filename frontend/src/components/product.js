...
const Product = () => {
  ...
  const handleLookup = async () => {
    ...
    const data = await axios.post(`https://egycsmq8p4.execute-api.us-east-1.amazonaws.com/dev/lookup/products?url=https://pfm-sv.s3.amazonaws.com/${filename}`)
  ...
  const handleUpdate = async () => {
    ...
    await s3.upload(params).promise();
    axios.post(`https://egycsmq8p4.execute-api.us-east-1.amazonaws.com/dev/update/products?url=https://pfm-sv.s3.amazonaws.com/${filename}`)
      .then(data => {
        notification['success']({
  ...
  const handleInit = async () => {
    setInitLoading(true)
    await axios.post(`https://egycsmq8p4.execute-api.us-east-1.amazonaws.com/dev/initialize/products`)
  ...
  return (
    <div className="bg-white p-20 shadow-lg rounded-lg">
      <h1 className="text-4xl font-bold mb-5">BB Product Task</h1>
